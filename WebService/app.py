...
#(1) 업체명 (~원)
with col1 :
    cost = st.text_input('업체명을 입력해주세요.', value="")

#(2) 카테고리 (건축~)
col1,empty2,col2 = st.columns([1, 0.03, 1])
with col1 :
    facility = ['선택해주세요', '문화/생활', '여행/해외', '쇼핑/무이자', '정기결제', '할인/캐시백', '응모/경품', 'QR 결제']
...
#(3) 이벤트 명 (~원)
with col1 :
    cost = st.text_input('이벤트 명(타이틀)을 입력해주세요.', value="")
    
#(4) 광고기간 (yy.mm.dd ~ yy.mm.dd)
col1,empty2,col2 = st.columns([1, 0.03, 1])
with col1 :
    start_day = st.date_input('광고 시작일을 선택해주세요.')
with col2 :
...
df.loc['광고 기간'] = dur
    ...
    #(6) 광고 대상자
with col1 :
    cost = st.text_input('광고 대상자를 입력해주세요.', value="")



#(7) 광고 텍스트 (~원)
with col1 :
    cost = st.text_input('광고 텍스트를 입력해주세요.', value="")
